...
Every checked-in card has an adjacent `*.metadata.json` renderer input. The documentation tests render all nine metadata files with `cards.py` and require byte-for-text equality with their Markdown cards, so a card cannot drift from its reproducible input. The renderer uses exclusive creation for ad hoc output and will not overwrite an existing evidence artifact.
